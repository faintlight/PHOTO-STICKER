...
demoimg=Image.open("avatar//1.jpg")
ls=os.listdir('avatar')
print('the total number of photos is '+str(len(ls)))
target_origin_h=demoimg.size[1]
target_origin_w=demoimg.size[0]
print(target_origin_w)
print(target_origin_h)
...
# root: 脚本的根目录
root=""
W_num =  int(math.sqrt(len(ls)))
H_num = int(math.sqrt(len(ls)))
W_size = int(target_origin_w)
H_size = int(target_origin_h)
...
def createNevImg():
    ...
    print(root)
    I = numpy.array(transfer(root+"targetimg.jpg", iW_size, iH_size)) * 1.0
    ...
    img=img.convert('RGB')
    img.save("basic_combination\createNevImg_past.jpg")
...
def newRotateImage():
    imName = "basic_combination\createNevImg_past.jpg"
    print("正在将图片旋转中...")
    STA = time.time()
    ...
    im2 = im2.rotate(359)
    im2=im2.convert('RGB')
    im2.save("basic_combination\ewRotateImage_past.jpg")
    print("basic_combination\ewRotateImage Func Time %s"%(time.time()-STA))
...
def writeToImage():
    print("正在向图片中添加祝福语...")
    STA = time.time()
    img = Image.open("basic_combination\ewRotateImage_past.jpg")
...
#    draw.text((0,H_size * 6),unicode("happy every day",'utf-8'),(0,0,0),font=font)
    ...
    img.save("basic_combination\Final_past.jpg")
    print("basic_combination\writeToImage Func Time %s"%(time.time()-STA))
