...
def createNevImg():
    ...
    I = numpy.array(transfer(root+"targetimg.jpg", iW_size, iH_size))
    ...
    Image.fromarray(I.astype(numpy.uint8)).save("3_combination\createNevImg_%s_past3.png"%alpha)


# name: newRotateImage
# todo: 将createnevimg中得到的照片旋转，粘贴到另外一张照片中
def newRotateImage():
    imName = "3_combination\createNevImg_%s_past3.png" % alpha
    print("正在将图片旋转中...")
    im = Image.open(imName)
    im2 = Image.new("RGBA", (W_size * int(W_num + 1), H_size * (H_num + 4)))
    im2.paste(im, (int(0.5 * W_size), int(0.8 * H_size)))
    im2 = im2.rotate(359)
    im2.save("3_combination\RotatedImage_%s_past3.png"%alpha)
...
def writeToImage():
    print("正在向图片中添加祝福语...")
    img = Image.open("3_combination\RotatedImage_%s_past3.png" % alpha)
...
#    draw.text((0,H_size * 6),unicode("happy every day",'utf-8'),(0,0,0),font=font)
    ...
    img.save("3_combination\FINAL_%s_past3.png" % alpha)
